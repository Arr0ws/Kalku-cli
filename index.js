...
function algcalc() {
    ...
    function loop2(skipClear1 = false) {
        ...
        if (user1 === "quit") {
            ...
            user1 === "up,up,down,down,left,right,left,right,b,a,start"
        ) {
        } else {
            mainadv(user1);
        }
        ...
        function mainadv(user1) {
            ...
            try {
                ...
                } else if (user1.includes("Limit: ")) {
                    let expr = user1.slice(6).trim();
                    let [expression, variable, value] = expr
                        .split(",")
                        .map((s) => s.trim());
                    let eqin = Algebrite.run(
                        `subst(${value}, ${variable}, ${expression})`
                    );
                    console.log(`lim ${expr} =>`, eqin);
                } else {
